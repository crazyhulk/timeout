...
func main() {
	...
	g.Printf("\n")
	g.Printf("import \"git.bilibili.co/go/util/conf\"\n")
	g.Printf("import \"git.bilibili.co/go/util/ctxkit\"\n")
...
func (g *Generator) parsePackage(patterns []string, tags []string) {
				...
				if funcDecl, ok := decl.(*ast.FuncDecl); ok {
					...
					if funcDecl.Doc != nil {
						for _, comment := range funcDecl.Doc.List {
							...
							if strings.HasPrefix(comment.Text, "//go:generate") && strings.Contains(comment.Text, "timeout") {
								fmt.Printf("======%+v\n", len(funcDecl.Doc.List))
								pos := pkg.Fset.Position(funcDecl.Pos())
								fmt.Printf("Found go:generate timeout in %s at line %d for function %s: %s, type:%+v\n",
	...
	// 					// 可以只查找包含 timeout 的指令
	// 					if strings.Contains(comment.Text, "timeout") {
	// 						pos := pkg.Fset.Position(comment.Pos())
	// 						fmt.Printf("Found go:generate timeout in %s at line %d: %s\n", pos.Filename, pos.Line, comment.Text)
	// 					}
	// 				}
	// 			}
	// 		}
...
// 获取函数签名的字符串表示
func getFuncSignature(fn *ast.FuncDecl, suffix string) string {
	// 获取函数名称
	...
	if len(results) == 0 {
		return fmt.Sprintf("func %s(%s)", funcName+suffix, strings.Join(params, ", "))
	} else if len(results) == 1 {
		return fmt.Sprintf("func %s(%s) %s", funcName+suffix, strings.Join(params, ", "), results[0])
	}

	// 拼接函数签名
	return fmt.Sprintf("func %s(%s) (%s)", funcName+suffix, strings.Join(params, ", "), strings.Join(results, ", "))
}
...
func (g *Generator) genTimeFunc(fn *ast.FuncDecl) {
	...
	g.Printf("%s {\n", getFuncSignature(fn, "WithTimeout"))
	g.Printf("\tyes := conf.GetBool(\"ENABLE_BIZ_DOWNGRADE\")\n")
	g.Printf("\tif yes {\n")
	...
	g.Printf("\tcase err := <-resultChan:\n")
	g.Printf("\t\t\treturn %s\n", strings.Join(res, ", "))
	g.Printf("\t}\n\n")
	g.Printf("\treturn %s\n", strings.Join(res, ", "))
	g.Printf("}\n")

	//		tmpl := `func {{Name}}WithTimeout(ctx context.Context, timeout time.Duration, fn {{Sign}}) Resp {
	//		yes := conf.GetBool("ENABLE_BIZ_DOWNGRADE")
	//		if yes {
	//			return fn(ctx)
	//		}
	//
	//		traceID := ctx.Value(ctxkit.TraceIDKey)
	//
	//		nctx := context.Background()
	//		nctx = context.WithValue(nctx, ctxkit.TraceIDKey, traceID)
	//		// 创建一个有超时的上下文
